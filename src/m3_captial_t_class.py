...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        self.h_rect = rg.Rectangle(rg.Point(self.center.x - (width / 2), self.center.y - letter_thickness / 2),
                              rg.Point(self.center.x + (width / 2), self.center.y + letter_thickness / 2))
        self.v_rect = rg.Rectangle(rg.Point(self.center.x - (.5 * letter_thickness), self.center.y - (.5 * letter_thickness)),
                              rg.Point(self.center.x + (.5 * letter_thickness), (self.center.y - (.5 * letter_thickness) + height)))
    ...
    def clone(self):
        ...
        #     clone.pdf.
        # --------------------------------------------------------------
        t = CapitalT(self.center, self.width, self.height, self.letter_thickness)
